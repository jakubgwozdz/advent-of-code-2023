...
typealias Key = Pair<Pos, Boolean?>
...
data class State(val pos: Pos, val lastMove: Dir?, val cost: Int) : Comparable<State> {
    ...
    val key: Key = pos to lastMove?.let { it == E || it == W }
}
...
fun Input.solve(minMoves: Int, maxMoves: Int): Int? {
    ...
    fun testChecking(s: State) = besties[s.key].let { b -> b == null || b > s.cost }
    fun moves(s: State) = buildList {
    ...
    return search(
        ...
        ::moves,
        State::cost,
        { testChecking(it).also { better -> if (better) besties[it.key] = it.cost } },
        { it.pos.r == lastIndex && it.pos.c == last().lastIndex },
        PriorityQueue(compareBy { it.cost }),
    )
}
...
fun <T : Any, R : Comparable<R>> search(
    ...
    while (toGo.isNotEmpty()) {
        ...
        if (isWorthChecking(next) && (min == null || value < min)) {
